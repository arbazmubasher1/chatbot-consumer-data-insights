...
def prepare_data(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    # Parse key timestamps (we use Updated At everywhere for time features)
    time_cols = [
        "Updated At", "First Public Reply At", "First Private Reply At",
    ...
    # --- All time features derived from UPDATED AT ---
    updated = pd.to_datetime(df.get("Updated At"), errors="coerce")
    df["Date"] = updated.dt.date
    df["Hour"] = updated.dt.hour
    df["DayOfWeek"] = updated.dt.day_name()
    df["IsWeekend"] = df["DayOfWeek"].isin(["Saturday", "Sunday"])

    # ISO week fields + real datetime WeekStart (Monday)
    iso = updated.dt.isocalendar()
    df["ISO_Year"] = iso.year
    df["ISO_Week"] = iso.week
    df["WeekStart"] = (updated - pd.to_timedelta(updated.dt.weekday, unit="D")).dt.normalize()

    # Shifts (10–16, 16–22, 22–03, else catch-all)
    def shift_label(h):
        if pd.isna(h):
            return np.nan
        h = int(h)
...
with tabs[1]:
    ...
    # Weekday vs Weekend volume + Demoter % + NPS (using Updated At-derived fields)
    required = {"IsWeekend", "Feedback Head", "Ticket number"}
    ...
    # Weekly Response Trend (ISO) based on Updated At
    ...
    if "SLA Breach" in filtered.columns:
        ...
        if "Shift" in filtered.columns:
            sla_shift = filtered.pivot_table(index="Shift", values="SLA Breach",
                                             aggfunc=lambda s: np.mean(s.fillna(False).astype(float))*100)
            sla_shift = sla_shift.reindex(shift_order).fillna(0)
...
with tabs[3]:
    ...
    if set(["Tags","Shift","Ticket number"]).issubset(filtered.columns):
        ...
        if not tag_shift.empty:
            ts = tag_shift.pivot_table(index="Tags", columns="Shift", values="Ticket number",
                                       aggfunc="count", fill_value=0)
            ts = ts.reindex(columns=shift_order)
    ...
    texts = filtered["Description"].dropna().astype(str).tolist() if "Description" in filtered.columns else []
    ...
    if not PHONE_COL and not NAME_COL and not (ADDR_COL or ADDRESS_PROXY_COL):
        ...
        if group_keys:
            agg_parts = {
                ...
                "First Seen": ("Updated At", "min") if "Updated At" in cust_subset.columns else (TICKET_COL or "Date", "count"),
                "Last Seen": ("Updated At", "max") if "Updated At" in cust_subset.columns else (TICKET_COL or "Date", "count"),
            }
        ...
        if PHONE_COL and "_PhoneNorm" in cust_subset.columns and "Branch Name" in cust_subset.columns:
            ...
            pairs = mm.groupby(["Branch Name", "_PhoneNorm"]).agg(**({
                ...
                **({"FirstSeen": ("Updated At", "min"), "LastSeen": ("Updated At", "max")} if "Updated At" in mm.columns else {})
            })).reset_index()
            ...
            if repeats.empty:
                ...
                branch_roll = repeats.groupby("Branch Name").agg(
                    RepeatCustomers=("_PhoneNorm", "nunique"),
                    TotalRepeatVisits=("Visits", "sum")
                ).reset_index()
        ...
        ADDR_USED = ADDR_COL or ADDRESS_PROXY_COL
        compl_cols = [c for c in [TICKET_COL, NAME_COL, PHONE_COL, ADDR_USED, "Updated At",
                                  "Feedback Head", "Complaint Head", "Tags", "Description"] if c]
        compl_view = cust_subset.copy()
        if "Updated At" in compl_view.columns:
            compl_view = compl_view.sort_values("Updated At", ascending=False)
        if compl_cols:
        ...
        if go_btn and lookup_number and PHONE_COL:
            ...
            if len(uniq_phones) == 1:
                single_mask = True
                one = cust_subset.sort_values("Updated At")
        ...
        if single_mask:
            if "Updated At" in one.columns:
                tl = one.groupby(one["Updated At"].dt.date).size().reset_index(name="Rows")
                fig_tl = px.line(tl, x="Updated At", y="Rows", markers=True, title="Chronological Ticket Counts")
                st.plotly_chart(fig_tl, use_container_width=True)

            cols = [c for c in [TICKET_COL,"Updated At","Branch Name","Tags","Feedback Head","Description","SLA Breach","Re-Opened","TTR_min","FRT_min"] if c and c in one.columns]
            if cols:
...
with tabs[6]:
    ...
    cols = [c for c in ["Ticket number","Branch Name","Tags","Shift","Feedback Head","FRT_min","TTR_min","Updated At","Closed At"] if c in out.columns]
    if cols:
...
with tabs[7]:
    ...
    if set(["Updated At","First Public Reply At"]).issubset(filtered.columns):
        add_rule("Updated At ≤ First Public Reply At", (filtered["Updated At"] > filtered["First Public Reply At"]))
    ...
    if set(["Updated At","Closed At"]).issubset(filtered.columns):
        add_rule("Updated At ≤ Closed At", (filtered["Updated At"] > filtered["Closed At"]))
...
with tabs[8]:
    ...
    if "audit_payload" not in locals():
        ...
        if excl_count > 0:
            ...
            cols = [c for c in [
                "Ticket number","Updated At","Branch Name","Shift","DayOfWeek",
                "Tags","Feedback Head","Description"
            ] if c in pre.columns]
            if cols:
                st.dataframe(affected[cols].sort_values("Updated At").reset_index(drop=True),
